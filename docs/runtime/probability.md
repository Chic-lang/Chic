...
- Chic-native implementations live in `packages/std/src/probability/dist.ch` and use the shared `Std.Random.RNG` surface for deterministic sampling/log-prob accumulation across backends.
...
- `chic test packages/std/src/probability/dist.ch` exercises deterministic sampling and log-prob accumulation.
