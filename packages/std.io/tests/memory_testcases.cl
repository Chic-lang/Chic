...
testcase Given_memory_invalid_range_negative_start_throws_When_executed_Then_memory_invalid_range_negative_start_throws()
{
    Assert.Throws <ArgumentOutOfRangeException >(() => {
        var data = new int[2]; let _ = new Memory <int >(data, - 1, 1);
    }
    );
}
...
{
    Assert.Throws <ArgumentOutOfRangeException >(() => {
        var data = new int[2]; let _ = new Memory <int >(data, 1, 2);
    }
    );
}
...
{
    Assert.Throws <ArgumentOutOfRangeException >(() => {
        var data = new int[2]; let _ = new ReadOnlyMemory <int >(data, 2, 1);
    }
    );
}
