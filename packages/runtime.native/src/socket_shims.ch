...
public static class SocketShims
{
    ...
    * const SockAddrIn6 addr, int addrlen);
    @extern("C") @export("accept6") public static int Accept6(int fd, * mut SockAddrIn6 addr, * mut int addrlen) {
        return accept(fd, addr, addrlen);
    }
    @extern("C") @export("bind6") public static int Bind6(int fd, * mut SockAddrIn6 addr, int addrlen) {
        return bind(fd, addr, addrlen);
    }
    @extern("C") @export("connect6") public static int Connect6(int fd, * mut SockAddrIn6 addr, int addrlen) {
        return connect(fd, addr, addrlen);
    }
    @extern("C") @export("recvfrom6") public static isize ReceiveFrom6(int fd, * mut @expose_address byte buffer, usize length,
    int flags, * mut SockAddrIn6 addr, * mut int addrlen) {
        return recvfrom(fd, buffer, length, flags, addr, addrlen);
    }
    @extern("C") @export("sendto6") public static isize SendTo6(int fd, * const @expose_address byte buffer, usize length,
    int flags, * const SockAddrIn6 addr, int addrlen) {
