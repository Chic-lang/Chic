...
testcase Given_vec_push_pop_insert_remove_When_executed_Then_vec_push_pop_insert_remove()
{
    unsafe {
        ...
        let ok = push1 == 0 && push2 == 0 && push3 == 0 && len1 == 3usize && insertStatus == 0 && len2 == 4usize && removeStatus == 0 && len3 == 3usize && popStatus == 0 && len4 == 2usize && clearStatus == 0 && empty == 1;
...
testcase Given_vec_iteration_and_views_When_executed_Then_vec_iteration_and_views()
{
    unsafe {
        ...
        let ok = viewStatus == 0 && pushOk && vecView.len == 3usize && firstStatus == 0 && firstValue == 5 && secondStatus == 0 && secondValue == 6 && thirdStatus == 0 && thirdValue == 7 && doneStatus == 6;
        Assert.That(ok).IsTrue();
        VecRuntime.chic_rt_vec_drop(& vec);
    }
}
testcase Given_vec_layout_and_inline_helpers_When_executed_Then_vec_layout_and_inline_helpers()
{
    unsafe {
        ...
        let ok = info.size >0usize && info.offset_inline_storage >0usize && inlineCap >0usize && !NativePtr.IsNull(inlinePtr.Pointer) && usesInline == 1;
        Assert.That(ok).IsTrue();
        VecRuntime.chic_rt_vec_drop(& vec);
    }
}
testcase Given_vec_clone_reserve_and_into_array_When_executed_Then_vec_clone_reserve_and_into_array()
{
    unsafe {
        ...
        let ok = reserveStatus == 0 && shrinkStatus == 0 && cloneStatus == 0 && cloneLen == 2usize && copyStatus == 0 && copyLen == 2usize && moveStatus == 0 && originalLen == 0usize && movedLen == 2usize;
        Assert.That(ok).IsTrue();
...
testcase Given_vec_ptr_access_and_errors_When_executed_Then_vec_ptr_access_and_errors()
{
    unsafe {
        ...
        let ok = reserveStatus == 2 && !NativePtr.IsNullConst(data.Pointer) && !NativePtr.IsNull(dataMut.Pointer) && !NativePtr.IsNull(ptrAt.Pointer) && NativePtr.IsNull(ptrOob.Pointer);
        Assert.That(ok).IsTrue();
        VecRuntime.chic_rt_vec_drop(& vec);
    }
}
testcase Given_vec_error_paths_and_zero_sized_When_executed_Then_vec_error_paths_and_zero_sized()
{
    unsafe {
        ...
        let ok = zeroPush == 0 && zeroLen == 1usize && popEmpty == (int) VecError.OutOfBounds && badInsert == (int) VecError.InvalidPointer && badRemove == (int) VecError.OutOfBounds && badSwap == (int) VecError.OutOfBounds && iterStatus == (int) VecError.InvalidPointer && viewStatus == (int) VecError.InvalidPointer;
        Assert.That(ok).IsTrue();
        VecRuntime.chic_rt_vec_drop(& vec);
    }
}
...
testcase Given_vec_truncate_set_len_capacity_and_swap_remove_When_executed_Then_vec_truncate_set_len_capacity_and_swap_remove()
{
    unsafe {
        ...
        let cloneLen = VecRuntime.chic_rt_vec_len(& clone);
        let ok = cap >= 2usize && elemSizeActual == elemSize && elemAlignActual == elemAlign && setLen1 == 0 && len1 == 1usize && truncateStatus == 0 && setLen2 == 0 && swapStatus == 0 && len2 == 1usize && copyStatus == 0 && cloneLen == 1usize;
        Assert.That(ok).IsTrue();
        VecRuntime.chic_rt_vec_drop(& clone);
        VecRuntime.chic_rt_vec_drop(& vec);
    }
...
testcase Given_vec_growth_and_truncate_When_executed_Then_vec_growth_and_truncate()
{
    unsafe {
        ...
        let ok = len == inlineCap + 3usize && cap >= inlineCap + 3usize && truncStatus == 0 && lenAfter == inlineCap;
        Assert.That(ok).IsTrue();
        VecRuntime.chic_rt_vec_drop(& vec);
    }
}
testcase Given_vec_setters_and_failure_paths_When_executed_Then_vec_setters_and_failure_paths()
{
    unsafe {
        ...
        let ok = empty == 1 && layout.size >0usize && usesInline == 1 && elemSizeActual == elemSize && elemAlignActual == elemAlign && invalidPush == (int) VecError.InvalidPointer && invalidPop == (int) VecError.InvalidPointer && iterStatus == (int) VecError.IterationComplete && reserveFail == (int) VecError.AllocationFailed;
        Assert.That(ok).IsTrue();
        VecRuntime.chic_rt_vec_drop(& vec);
    }
}
testcase Given_vec_new_in_region_and_array_into_vec_When_executed_Then_vec_new_in_region_and_array_into_vec()
{
    unsafe {
        ...
        VecRuntime.chic_rt_vec_drop(& vec);
        chic_rt_region_exit(regionHandle);
        let ok = pushStatus == 0 && moveStatus == 0 && movedLen == 1usize && originalLen == 0usize;
        Assert.That(ok).IsTrue();
    }
}
