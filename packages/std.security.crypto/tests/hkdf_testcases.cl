...
private static byte[] Case1Info() {
    return new byte[10] {
        0xF0u8, 0xF1u8, 0xF2u8, 0xF3u8, 0xF4u8,
        0xF5u8, 0xF6u8, 0xF7u8, 0xF8u8, 0xF9u8,
    };
}
...
private static byte[] Case3Prk() {
    return new byte[32] {
        0x19u8, 0xEFu8, 0x24u8, 0xA3u8, 0x2Cu8, 0x71u8, 0x7Bu8, 0x16u8,
        0x7Fu8, 0x33u8, 0xA9u8, 0x1Du8, 0x6Fu8, 0x64u8, 0x8Bu8, 0xDFu8,
        0x96u8, 0x59u8, 0x67u8, 0x76u8, 0xAFu8, 0xDBu8, 0x63u8, 0x77u8,
        0xACu8, 0x43u8, 0x4Cu8, 0x1Cu8, 0x29u8, 0x3Cu8, 0xCBu8, 0x04u8,
    };
}
...
testcase Given_hkdf_extract_sha256_known_vector_When_executed_Then_hkdf_extract_sha256_known_vector()
{
    ...
    let salt = Case1Salt();
    var prk = new byte[32];
    let written = HKDF.Extract(HashAlgorithmName.Sha256(), ReadOnlySpan<byte>.FromArray(in ikm),
        ReadOnlySpan<byte>.FromArray(in salt), Span<byte>.FromArray(ref prk));
...
testcase Given_hkdf_expand_sha256_known_vector_When_executed_Then_hkdf_expand_sha256_known_vector()
{
    ...
    let info = Case1Info();
    var okm = new byte[42];
    let written = HKDF.Expand(HashAlgorithmName.Sha256(), ReadOnlySpan<byte>.FromArray(in prk),
        ReadOnlySpan<byte>.FromArray(in info), Span<byte>.FromArray(ref okm));
...
testcase Given_hkdf_extract_empty_salt_matches_vector_When_executed_Then_hkdf_extract_empty_salt_matches_vector()
{
    ...
    var prk = new byte[32];
    let written = HKDF.Extract(HashAlgorithmName.Sha256(), ReadOnlySpan<byte>.FromArray(in ikm),
        ReadOnlySpan<byte>.Empty, Span<byte>.FromArray(ref prk));
    Assert.That(written).IsEqualTo(32);
...
testcase Given_hkdf_extract_rejects_short_destination_When_executed_Then_hkdf_extract_rejects_short_destination()
{
    ...
    Assert.Throws<ArgumentException>(() => {
        let _ = HKDF.Extract(HashAlgorithmName.Sha256(), ReadOnlySpan<byte>.FromArray(in ikm),
            ReadOnlySpan<byte>.FromArray(in salt), Span<byte>.FromArray(ref prk));
    });
}
...
testcase Given_hkdf_expand_rejects_short_prk_When_executed_Then_hkdf_expand_rejects_short_prk()
{
    ...
    Assert.Throws<ArgumentException>(() => {
        let _ = HKDF.Expand(HashAlgorithmName.Sha256(), ReadOnlySpan<byte>.FromArray(in prk),
            ReadOnlySpan<byte>.Empty, Span<byte>.FromArray(ref output));
    });
}
...
{
    let ikm = Case1Ikm();
    let derived = HKDF.DeriveKey(HashAlgorithmName.Sha256(), ReadOnlySpan<byte>.FromArray(in ikm),
        ReadOnlySpan<byte>.Empty, ReadOnlySpan<byte>.Empty, 0);
...
testcase Given_hkdf_derive_key_negative_length_throws_When_executed_Then_hkdf_derive_key_negative_length_throws()
{
    ...
    Assert.Throws<ArgumentOutOfRangeException>(() => {
        let _ = HKDF.DeriveKey(HashAlgorithmName.Sha256(), ReadOnlySpan<byte>.FromArray(in ikm),
            ReadOnlySpan<byte>.Empty, ReadOnlySpan<byte>.Empty, -1);
    });
}
