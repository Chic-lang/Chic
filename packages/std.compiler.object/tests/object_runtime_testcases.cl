...
testcase Given_object_runtime_size_matches_intrinsic_When_executed_Then_object_runtime_size_matches_intrinsic()
{
    ...
    unsafe {
        var entry = new TypeMetadataEntry {
            type_id = typeId,
            size = __sizeof<ObjectRuntime>(),
            align = __alignof<ObjectRuntime>(),
            drop_fn = 0isize,
            variance = new Std.Runtime.VarianceSlice { Ptr = null, Len = 0usize },
            flags = 0u,
        };
...
testcase Given_object_runtime_hash_eq_glue_tables_When_installed_Then_lookup_returns_indices()
{
    ...
    unsafe {
        var entryA = new GlueIndexEntry { type_id = typeA, function_index = 42u };
        ...
        var entryB = new GlueIndexEntry { type_id = typeB, function_index = 7u };
...
testcase Given_object_runtime_drop_glue_When_installed_Then_queries_return_value()
{
    ...
    unsafe {
        var entry = new TypeMetadataEntry {
            type_id = typeA,
            size = __sizeof<SampleA>(),
            align = __alignof<SampleA>(),
            drop_fn = 55isize,
            variance = new Std.Runtime.VarianceSlice { Ptr = null, Len = 0usize },
            flags = 0u,
        };
