...
public static class ObjectRuntime
{
    ...
    private static unsafe isize LookupGlueHandle(* const @readonly @expose_address GlueIndexEntry table, usize len, u64 type_id) {
        ...
        while (i <len)
        {
            let entryPtr = (* const @readonly @expose_address GlueIndexEntry) OffsetConstLocal(AsConstByte(table),
            (isize)(i * sizeof(GlueIndexEntry)));
            if ( (* entryPtr).type_id == type_id)
            {
    ...
    // --- ABI exports (weak on native; used by WASM executor for delegation) ---
	    ...
	    @extern("C") @weak @export("chic_rt_type_metadata") public unsafe static int chic_rt_type_metadata(u64 type_id,
	    * mut @expose_address Std.Runtime.TypeMetadataRecord out_metadata) {
