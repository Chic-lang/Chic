...
use crate::frontend::ast::{
    CrateAttributes, CrateStdSetting, Item, Module as AstModule, NamespaceDecl,
};
...
use crate::manifest::{
    DependencySource, DocEnforcementScope, DocEnforcementSeverity, Manifest, MissingDocsRule,
    SourceRoot, WorkspaceConfig,
};
use crate::mir::normalise_cost_model;
use crate::mir::{
    Abi, BorrowCheckResult, FunctionKind, InterfaceDefaultImpl, LoweringDiagnostic, LoweringResult,
    MirModule, borrow_check_module, check_fallible_values, check_unreachable_code,
    configure_pointer_width, intern_raw_strings, lower_module_with_units_and_hook, verify_body,
};
...
impl<'a> CompilerPipeline<'a> {
    pub fn execute(&self) -> Result<FrontendState> {
        ...
        let workspace_crate_attributes =
            resolve_workspace_crate_attributes(&mut modules, self.config.manifest.as_ref());
        ...
        let used_packages = collect_used_packages(&modules);
        ...
        if self.config.restore_enabled {
            if let Some(manifest) = &self.config.manifest {
                if let Some(manifest_path) = manifest.path() {
                    ...
                    let reachable = compute_reachable_packages(&used_packages, &resolved);
                    let mut names: Vec<_> = resolved.keys().cloned().collect();
                    names.sort();
                    for name in names {
        ...
        let doc_diagnostics = if suppress_bootstrap_diagnostics {
            ...
            enforce_missing_docs(&modules, &self.config.doc_enforcement)
        };

        let trim_stats = if self.config.manifest.is_some() {
            Some(trim_unreachable_package_exports(
...
fn collect_used_packages(modules: &[FrontendModuleState]) -> HashSet<String> {
...
#[derive(Default)]
struct TrimStats {
    trimmed_functions: usize,
    trimmed_exports: usize,
}

fn trim_unreachable_package_exports(
    for_tests: bool,
