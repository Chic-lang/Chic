...
body_builder_impl! {
    fn drop_glue_zero_operand() -> Operand {
        Operand::Const(ConstOperand::new(ConstValue::Null))
    }

    pub(crate) fn try_lower_drop_glue_expr(
        &mut self,
        expr: &ExprNode,
        span: Option<Span>,
            ) -> Option<Operand> {
        let mut tokens = Vec::new();
        if Self::collect_drop_glue_tokens(expr, &mut tokens) {
            match Self::parse_drop_glue_tokens(&tokens) {
                GlueParseResult::Success { type_text } => {
                    let operand = self.lower_drop_glue_for_type(&type_text, span);
                    return Some(operand);
                }
                GlueParseResult::RuntimeArgs => {
                    self.diagnostics.push(LoweringDiagnostic {
                        message: "`__drop_glue_of` does not accept runtime arguments".into(),
                        span,
                                            });
                    return Some(Self::drop_glue_zero_operand());
                }
                GlueParseResult::MissingType => {
                    self.diagnostics.push(LoweringDiagnostic {
                        message: "`__drop_glue_of` requires a type argument (e.g. `__drop_glue_of<MyType>()`)".into(),
                        span,
                                            });
                    return Some(Self::drop_glue_zero_operand());
                }
        ...
        } = expr
        {
                ...
                && name.trim() == "__drop_glue_of"
            {
                    ...
                    && let Some(first) = generics.first()
                {
                    ...
                    let operand = self.lower_drop_glue_for_type(first.trim(), span);
                    return Some(operand);
                }
            }
        }
        ...
        if let ExprNode::Call { callee, args, .. } = expr {
            return self.try_lower_drop_glue_intrinsic(callee, args, span);
        }

        None
    }
    pub(crate) fn try_lower_drop_glue_intrinsic(
        &mut self,
        callee: &ExprNode,
        args: &[CallArgument],
        span: Option<Span>,
        ...
        const INTRINSIC_NAME: &str = "__drop_glue_of";
        ...
        Some(Self::drop_glue_zero_operand())
    }
    pub(crate) fn lower_drop_glue_for_type(
        &mut self,
        type_text: &str,
        span: Option<Span>,
            ) -> Operand {
        ...
        if self.type_param_name_for_ty(&ty).is_some() {
            if let Some(operand) = self.runtime_type_drop_operand(&ty, span) {
                return operand;
            }
            return Self::drop_glue_zero_operand();
        }
        ...
        if !self.type_layouts.type_requires_drop(&layout_name) {
            return Self::drop_glue_zero_operand();
        }

        let symbol = drop_glue_symbol_for(&layout_name);
        Operand::Const(ConstOperand::new(ConstValue::Symbol(symbol)))
    }
    pub(crate) fn collect_drop_glue_tokens(expr: &ExprNode, out: &mut Vec<GlueToken>) -> bool {
        match expr {
            ...
            ExprNode::Binary { op, left, right } => {
                ...
                if !Self::collect_drop_glue_tokens(left, out) {
                    return false;
                }
                match symbol {
                    ">>" => {
                ...
                Self::collect_drop_glue_tokens(right, out)
            }
            ExprNode::Parenthesized(inner) => {
                out.push(GlueToken::Symbol("("));
                if !Self::collect_drop_glue_tokens(inner, out) {
                    return false;
                }
                out.push(GlueToken::Symbol(")"));
                true
    ...
    pub(crate) fn parse_drop_glue_tokens(tokens: &[GlueToken]) -> GlueParseResult {
        if tokens.len() < 3 {
            return GlueParseResult::NotMatch;
        }

        match tokens.first() {
            Some(GlueToken::Ident(name)) if name == "__drop_glue_of" => {}
