use super::*;
use crate::hash_glue::hash_glue_symbol_for;

body_builder_impl! {
    pub(crate) fn try_lower_hash_glue_expr(
        &mut self,
        expr: &ExprNode,
        span: Option<Span>,
    ) -> Option<Operand> {
        let mut tokens = Vec::new();
        if Self::collect_hash_glue_tokens(expr, &mut tokens) {
            match Self::parse_hash_glue_tokens(&tokens) {
                GlueParseResult::Success { type_text } => {
                    let operand = self.lower_hash_glue_for_type(&type_text, span);
                    return Some(operand);
                }
                GlueParseResult::RuntimeArgs => {
                    self.diagnostics.push(LoweringDiagnostic {
                        message: "`__hash_glue_of` does not accept runtime arguments".into(),
                        span,
                    });
                ...
                GlueParseResult::MissingType => {
                    self.diagnostics.push(LoweringDiagnostic {
                        message: "`__hash_glue_of` requires a type argument (e.g. `__hash_glue_of<MyType>()`)".into(),
                        span,
                    });
                    return Some(Operand::Const(ConstOperand::new(ConstValue::Null)));
                }
        ...
        } = expr
        {
                ...
                && name.trim() == "__hash_glue_of"
            {
                    ...
                    && let Some(first) = generics.first()
                {
                    ...
                    let operand = self.lower_hash_glue_for_type(first.trim(), span);
                    return Some(operand);
                }
            }
        }
        ...
        if let ExprNode::Call { callee, args, .. } = expr {
            return self.try_lower_hash_glue_intrinsic(callee, args, span);
        }

        None
    }

    pub(crate) fn try_lower_hash_glue_intrinsic(
        &mut self,
        callee: &ExprNode,
        args: &[CallArgument],
        span: Option<Span>,
        ...
        const INTRINSIC_NAME: &str = "__hash_glue_of";
    ...
    pub(crate) fn lower_hash_glue_for_type(
        &mut self,
        type_text: &str,
        span: Option<Span>,
    ) -> Operand {
        let Some(type_expr) = parse_type_expression_text(type_text) else {
            self.diagnostics.push(LoweringDiagnostic {
                message: format!("`{type_text}` is not a valid type for `__hash_glue_of`"),
                span,
            });
        ...
        if self.type_param_name_for_ty(&ty).is_some() {
            if let Some(operand) = self.runtime_type_hash_operand(&ty, span) {
                return operand;
            }
            return Operand::Const(ConstOperand::new(ConstValue::Null));
        }
        ...
        let method_symbol = format!("{layout_name}::GetHashCode");
        if self
            .symbol_index
        ...
        let symbol = hash_glue_symbol_for(&layout_name);
        Operand::Const(ConstOperand::new(ConstValue::Symbol(symbol)))
    }

    pub(crate) fn collect_hash_glue_tokens(expr: &ExprNode, out: &mut Vec<GlueToken>) -> bool {
        match expr {
            ...
            ExprNode::Binary { op, left, right } => {
                ...
                if !Self::collect_hash_glue_tokens(left, out) {
                    return false;
                }
                match symbol {
                    ">>" => {
                ...
                Self::collect_hash_glue_tokens(right, out)
            }
            ExprNode::Parenthesized(inner) => {
                out.push(GlueToken::Symbol("("));
                if !Self::collect_hash_glue_tokens(inner, out) {
                    return false;
                }
                out.push(GlueToken::Symbol(")"));
                true
    ...
    pub(crate) fn parse_hash_glue_tokens(tokens: &[GlueToken]) -> GlueParseResult {
        if tokens.len() < 3 {
            return GlueParseResult::NotMatch;
        }

        match tokens.first() {
            Some(GlueToken::Ident(name)) if name == "__hash_glue_of" => {}
