...
use super::tracing::SchedulerTracer;
...
impl<'a> CallDepthGuard<'a> {
    fn new(exec: &mut Executor<'a>, func_index: u32) -> Self {
...
struct StackPointerGuard<'a> {
    exec: *mut Executor<'a>,
    saved: Option<i32>,
}

impl<'a> StackPointerGuard<'a> {
    fn new(exec: &mut Executor<'a>, stack_base: i32) -> Self {
        let mut saved = None;
...
fn adjust_rounding_f32(original: f64, nearest: f32, mode: RoundingMode) -> f32 {
...
fn adjust_rounding_f64(original: f64, nearest: f64, mode: RoundingMode) -> f64 {
...
fn convert_int_to_f32(value: i128, mode: RoundingMode) -> f32 {
...
fn convert_int_to_f64(value: i128, mode: RoundingMode) -> f64 {
...
fn round_f64_to_f32(value: f64, mode: RoundingMode) -> f32 {
    if value.is_nan() {
...
fn round_value(value: f64, mode: RoundingMode) -> f64 {
    match mode {
...
fn record_conversion_flags(value: f64, rounded: f64, min: f64, max: f64) {
    let mut flags = FloatStatusFlags::default();
    if value.is_nan() {
        flags.invalid = true;
        record_flags(flags);
...
fn record_arithmetic_flags(lhs: f64, rhs: f64, exact: f64, rounded: f64, is_div: bool) {
    let mut flags = FloatStatusFlags::default();
    if exact.is_nan() {
        flags.invalid = true;
    }
