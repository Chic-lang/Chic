...
3. **Document behavior.** If your change affects user-facing behavior, update `README.md`, inline help (`chic --help`), and/or the relevant docs under `docs/` (and `SPEC.md` when language semantics change).
...
- Run **all** checks locally: `cargo fmt -- --check`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo xtask lint-ll1`, `cargo xtask lint-runtime-shim`, and the relevant tests (often `cargo test --all --all-targets`).
...
If you run into problems setting up the project, would like feedback on an idea, or just want to say _hi_ - please open an issue (or a Discussion topic if enabled). We are happy to help.

Together we can make Chic an incredible language and toolchain. Happy hacking!
...
Have you discovered a vulnerability or have concerns about unsafe AI-assisted workflows? Please do not open a public issue. Use GitHub Security Advisories (preferred) or contact the maintainer via `https://chic-lang.com/` and we will respond promptly.
