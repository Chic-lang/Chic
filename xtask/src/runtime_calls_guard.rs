...
const BASELINE: &str = "docs/runtime/rust_runtime_callers.txt";
const SEARCH_ROOTS: &[&str] = &["src", "tests", "benches", "xtask", "packages"];
const SKIP_PREFIXES: &[&str] = &[
    "src/runtime",
    "packages/runtime.native",
    "packages/runtime.no_std",
    "runtime_adapter",
];
...
pub fn run() -> Result<(), Box<dyn Error>> {
    ...
    if adapter_root.exists() {
        return Err(
            "runtime_adapter folder must be removed; migrate runtime semantics to Chic".into(),
        );
    }
    ...
    if !additions.is_empty() {
        eprintln!(
            "lint-runtime-calls: new chic_rt_* references detected in Rust sources outside src/runtime:"
        );
        ...
        eprintln!(
            "Route language-visible behaviour through Std.* and the native runtime; \
             update the spec and baseline if a new runtime symbol is truly required."
        );
        return Err("lint-runtime-calls failed".into());
    }

    if strict {
        println!(
            "lint-runtime-calls: strict mode enabled; no chic_rt_* callsites remain outside src/runtime."
        );
    } else {
        println!(
            "lint-runtime-calls: runtime symbol references in Rust sources are unchanged ({} symbols).",
            current.len()
        );
    }
    Ok(())
...
fn load_baseline(path: &Path) -> Result<HashSet<String>, Box<dyn Error>> {
    let data = fs::read_to_string(path).map_err(|err| {
        format!(
            "failed to read runtime caller baseline {}: {err}",
            path.display()
        )
    })?;
    let set = data
...
fn collect_symbols(
    ...
    let re = Regex::new(r"chic_rt_[A-Za-z0-9](?:[A-Za-z0-9_]*[A-Za-z0-9])?")?;
