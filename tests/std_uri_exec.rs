...
#[test]
fn std_uri_executes_on_llvm() -> Result<(), Box<dyn Error>> {
    if !codegen_exec_enabled() {
        eprintln!("skipping std_uri_executes_on_llvm (set CHIC_ENABLE_CODEGEN_EXEC=1 to enable)");
        return Ok(());
    }
    if !clang_available() {
        eprintln!("skipping std_uri_executes_on_llvm (clang not available)");
        return Ok(());
    }
    build_and_execute_llvm(std_uri_program(), 0)
}

#[test]
fn std_uri_executes_on_wasm() -> Result<(), Box<dyn Error>> {
    if !codegen_exec_enabled() {
        eprintln!("skipping std_uri_executes_on_wasm (set CHIC_ENABLE_CODEGEN_EXEC=1 to enable)");
        return Ok(());
    }
    build_and_execute_wasm(std_uri_program(), 0)
}
