...
#[test]
fn std_math_executes_on_llvm() -> Result<(), Box<dyn Error>> {
    build_and_execute_llvm(std_math_program(), 0)
}

#[test]
fn std_math_executes_on_wasm() -> Result<(), Box<dyn Error>> {
    build_and_execute_wasm(std_math_program(), 0)
}
