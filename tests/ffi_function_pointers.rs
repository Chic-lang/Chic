...
#[test]
fn extern_function_pointers_round_trip_callbacks() {
    if !common::clang_available() {
        eprintln!("skipping ffi fnptr test: clang not available");
        return;
    }
    if cfg!(target_os = "windows") {
        eprintln!("skipping ffi fnptr test: static archive build not wired for windows yet");
        return;
    }

    let dir = tempdir().expect("temp dir");
    let c_path = dir.path().join("ffi_fnptr.c");
    common::write_source(&c_path, include_str!("ffi/fn_ptrs.c"));
    let _lib_path = build_static_library(dir.path(), "ffi_fnptr", &c_path);

    let chic_root = dir.path().join("ffi_fnptr_chic");
