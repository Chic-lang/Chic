...
#[test]
fn aggregate_returns_and_params_round_trip() {
    if !common::clang_available() {
        eprintln!("skipping ffi aggregate test: clang not available");
        return;
    }
    if cfg!(target_os = "windows") {
        eprintln!("skipping ffi aggregate test: static archive build not wired for windows yet");
        return;
    }

    let dir = tempdir().expect("temp dir");
    let c_path = dir.path().join("ffi_aggregates.c");
    common::write_source(&c_path, include_str!("ffi/aggregates.c"));
    let _lib_path = build_static_library(dir.path(), "ffi_aggregates", &c_path);

    let chic_root = dir.path().join("ffi_aggregate_returns_chic");
